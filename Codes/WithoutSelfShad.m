...
mm{3} = [0.0180235 0.09662902 0.206249947 0.308093869 0.399184709];      % methanotroph biomass (gDCW/L) at=140 umol/m2/hr
...
mmStd{2} = [0 0.002172932 0.002637890 0.001087146 0.001093641]; %standard deviation
mmStd{3} = [0 0.002811037 0.005277562 0.003303156 0.010272729]; %standard deviation
...
aa{3} = [0.2220025 0.469621049 0.747693536 1.006626101 1.20458353]; % Photoautotroph biomass (gDCW/L) at=140 umol/m2/hr.
...
% to be limitless...
CH4percentage = 1.8; %CH4 in gas
CO2percentage = 1.2; %CO2 in gas
O2percentage = 0.0; %O2 in gas
% Insert light intensity (umol/m2/s)(60/100/140/180):
N = 2; %Which experimental data set (light intensities: 1=60; 2=100; 3=140, 4=180)
if N==1
    I0=60;
elseif N==2
    I0=100;
...
n = 1;
Ksi = 35.33; % when we use a constant Ksi
...
%% 0-22.75 hr
tspan=[0:0.001:ttt(2)];
[T1,Y1] = ode45(@(t,y)functionWO(t,y,p),tspan,Y0);%No2
...
Y0(1)=Y1(end,1);
Y0(2)=Y1(end,2);
[T2,Y2] = ode45(@(t,y)functionWO(t,y,p),tspan,Y0);%No2
T2=T2+T1(end);
...
Y0(1)=Y2(end,1);
Y0(2)=Y2(end,2);
[T3,Y3] = ode45(@(t,y)functionWO(t,y,p),tspan,Y0);
T3=T3+T2(end);
...
Y0(1)=Y3(end,1);
Y0(2)=Y3(end,2);
[T4,Y4] = ode45(@(t,y)functionWO(t,y,p),tspan,Y0);%No2
T4=T4+T3(end);
...
rr{1}=[12.31739118  7.709250965 5.247164662 4.309247069 3.679209356];
rr{2}=[12.31739118  6.208108953 4.262672428 3.81497994 3.472154444];
rr{3}=[12.31739118  4.622055616 3.453253622 3.218835513 2.920188724];
rr{4}=[12.31739118  4.350453709 3.500363393 3.063831603 2.771165668];
rr{5}=[12.28717688  4.219503298 3.636384478 3.302734418 3.093705747];
