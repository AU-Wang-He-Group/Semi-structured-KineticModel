...
tt{4} = [0 22.75 46.75 70.75 94];
tt{5} = [0 22.75 46.75 70.75 94];%time
mm{1} = [0.01819575 0.027123679 0.041503693 0.097494607 0.196882834]; % methanotroph biomass (gDCW/L) at ratio 1.5:1
mm{2} = [0.050751   0.130876458 0.245510788 0.381786728 0.471952386]; % methanotroph biomass (gDCW/L) at ratio 3:1
mm{3} = [0.01819575 0.052293095 0.100581669 0.211556291 0.314165181];      % methanotroph biomass (gDCW/L) at ratio 4:1
mm{4} = [0.01819575 0.091072733 0.178128773 0.306920756 0.405720018]; % methanotroph biomass (gDCW/L) at ratio 8.5:1
mm{5} = [0.0180235  0.111752842 0.238617699 0.382078957 0.505804087]; % methanotroph biomass (gDCW/L) at ratio 12.5:1

mmStd{1} = [0 9.21854E-05 0.004898803 0.013932441 0.018542928]; %standard deviation
mmStd{2} = [0 0.011736976 0.02270413  0.018220134 0.022361662]; %standard deviation
mmStd{3} = [0 0.002708581 0.004922449 0.002794844 0.002980078]; %standard deviation
mmStd{4} = [0 0.0184383   0.040245925 0.023223035 0.024588569]; %standard deviation
mmStd{5} = [0 0.01951332  0.015941648 0.01985122  0.033642293]; %standard deviation

aa{1} = [0.023395  0.0676678   0.123095789 0.269983025 0.555524497]; % Photoautotroph biomass (gDCW/L) at ratio 1.5:1 
aa{2} = [0.15307   0.35978689  0.668885089 1.04308523  1.3160436193]; % Photoautotroph biomass (gDCW/L) at ratio 3:1
aa{3} = [0.07117   0.181577513 0.335740593 0.627240323 0.925206505]; % Photoautotroph biomass (gDCW/L) at ratio 4:1
aa{4} = [0.1510225 0.356493527 0.601992805 0.909339909 1.197337925]; % Photoautotroph biomass (gDCW/L) at ratio 8.5:1
aa{5} = [0.2220025 0.486175566 0.835248658 1.170625584 1.40166692]; % Photoautotroph biomass (gDCW/L) at ratio 12.5:1

aaStd{1} = [0 0.003124351 0.032510945 0.06064455  0.070965952]; %standard deviation
aaStd{2} = [0 0.051331247 0.032773551 0.052025682 0.070516101]; %standard deviation
aaStd{3} = [0 0.008954408 0.026135291 0.027521763 0.0144062]; %standard deviation
aaStd{4} = [0 0.00458466  0.057897276 0.042496708 0.066276703]; %standard deviation
aaStd{5} = [0 0.029877510 0.037920792 0.071175724 0.098381021 ]; %standard deviation


% Amount of gas consumption/production (every 24 hours)(mmol/L):
CO2pr24{1}=[0.03548	    0.080521003	0.19540433	0.340703418];
CO2pr24{2}=[0.23877675	0.548383923	0.57705624	0.421939675];
CO2pr24{3}=[0.11422265	0.233961015	0.436684868	0.450880338];
CO2pr24{4}=[0.3070674	0.415481265	0.591893618	0.431410725];
CO2pr24{5}=[0.323971462 0.525665813 0.488920498 0.301382765];

CO2co24{1}=[0.07565452	0.170313649	0.428535222	0.8275192];
CO2co24{2}=[0.606189773	1.127806575	1.332467884	0.913066315];
CO2co24{3}=[0.3018682	0.532935761	0.952011506 1.008954703];
CO2co24{4}=[0.693878038	0.936986112	1.334828177	0.972909953];
CO2co24{5}=[0.741957491 1.206873153 1.012279923 0.506164635];

O2pr24{1}=[0.09691344	0.218171785	0.53672362	0.932032095];
O2pr24{2}=[0.6397995	1.484220222	1.57705136	1.13004795];
O2pr24{3}=[0.3155321	0.64922071	1.174867395	1.24032097];
O2pr24{4}=[0.888857767	1.20027921	1.709914895	1.24629765];
O2pr24{5}=[0.88544738   1.425935099 1.3069639   0.817014026];

O2co24{1}=[0.0967214    0.218171785	0.53672362	0.932032095];
O2co24{2}=[0.6397995	1.484220222	1.57705136	1.13004795];
O2co24{3}=[0.3155321	0.64922071	1.174867395	1.24032097];
O2co24{4}=[0.8870836	1.20027921	1.709914895	1.24629765];
O2co24{5}=[0.87531485   1.425935099 1.3069639   0.817014026];

CH4co24{1}=[0.0744	    0.16782445	0.4097874	0.72156315];
CH4co24{2}=[0.490615	1.13863094	1.20823472	0.8754215];
CH4co24{3}=[0.242717	0.4932467	0.90374415	0.95640075];
CH4co24{4}=[0.682372	0.9232917	1.31531915	0.9586905];
CH4co24{5}=[0.671328	1.0942595	1.0056785	0.627658];
...
measCH4{2}= [32.3649  29.6319	32.3633	25.2902 32.33524  24.0933	32.3362	 26.1109];
measCH4{3}= [32.3395  30.4381	32.3109	29.2163	32.34783  26.6269	32.3613  25.9007];
measCH4{4}= [32.3359  28.2335	32.3643	26.2383	32.31403  23.8281	32.3662	 26.1811];
measCH4{5}= [32.3604  28.2228   32.3554	24.6505	32.3719	  26.0772	32.3631	 27.6395];
measCO2{1} = [5.353282352  3.880612649	5.302881249	3.918608204	5.234793215	4.088543322	5.211387953	4.160886858];
measCO2{2} = [5.283731489  3.751286548	5.244368095	3.552574789	5.317775506	3.672160069	5.311392253	4.170675373];
measCO2{3} = [5.294370244  3.956423188	5.226282211	3.918608204	5.172024558	3.771521313	5.228409962	3.651287997];
measCO2{4} = [5.283731489  3.91923513	5.277956165	3.875601536	5.272332823	3.581615943	5.283579507	4.060739176];
measCO2{5} = [5.519795244  4.128512157  5.413652589 3.228463662 5.407671583 4.015673199 5.499633596 4.3317370512];
measO2{1} = [0 0 0 0 0 0 0 0];
measO2{2} = [0 0 0 0 0 0 0 0];
measO2{3} = [0 0 0 0 0 0 0 0];
measO2{4} = [0 0 0 0 0 0 0 0];
measO2{5} = [0 0.134154 0 0 0 0 0 0];
...
%% initialization
%...
N =5; %Which experimental data set (population gDCW Ratios ( photoautotroph:methanotroph )): 1=1.5:1; 2=3:1; 3=4:1, 4=8.5:1, 5=12.5:1)
...
ttr{5} = [0 23.5 47.125 71.625 94.167];
rr{1}=[1.285739802 2.494786927 2.965899664 2.769209847 2.821599461];
rr{2}=[3.016098205 2.749057359 2.724463126 2.732114957 2.788509304];
rr{3}=[3.911352926 3.472303803 3.337989868 2.964886182 2.944968317];
rr{4}=[8.299877719 3.914382673 3.379537146 2.962784017 2.95114333];
rr{5}=[12.31739118 4.350453709 3.500363393 3.063831603 3.071165668];
